...
def get_contours(img_dilate,img_contours):
   ...
   for cnt in contours:
       ...
       cv2.drawContours(img_contours,cnt,-1,(0,255,0),5)
       peri=cv2.arcLength(cnt,True)
       approx=cv2.approxPolyDP(cnt,0,0.02*peri,True)
       x,y,w,h=cv2.boundingRect(approx)
       cv2.rectangle(img_contours,(x,y),(x+w,y+h),(255,0,255),5)
       cv2.putText(img_contours,"Points"+str(len(approx)),(x,y),(x+w,y+h),cv2.FONT_HERSHEY_COMPLEX,0.7,(255,0,255),2,0)
       cv2.putText(img_contours, "Area" + str(int(area_min)), (x, y), (x + w+45, y + h+45), cv2.FONT_HERSHEY_COMPLEX, 0.7,(255, 0, 255), 2, 0)

while(True):
    ret,img=cap.read()
    img_contours=img.copy()
    img_blur=cv2.GaussianBlur(img,(7,7),4)# (src,extend,intensity)
    img_gray=cv2.cvtColor(img_blur,cv2.COLOR_BGR2GRAY)
    t1=cv2.getTrackbarPos("threshold1","parameters")
    t2=cv2.getTrackbarPos("threshold2","parameters")
    img_canny=cv2.Canny(img_gray,t1,t2)#(src,threshold value1,threshold value2)
    kernel=np.ones((5,5),np.uint8)
    img_dilate=cv2.dilate(img_canny,kernel,iterations=1,)
    # cv2.imshow("output",img)
    # cv2.imshow("Image Blur",img_blur)
    #output=np.hstack([img,img_blur])  #the two frame get combined i.e img and img_blur or to output multiple screen
    #output = np.hstack([img_gray])
